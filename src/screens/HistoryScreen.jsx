...
function moodTrend(current, previous) {
  ...
  if (Math.abs(diff) < 0.4) return 'About the same as the previous week'
  return diff > 0 ? 'Felt better than the previous week' : 'Felt tougher than the previous week'
}
...
export default function HistoryScreen({ night }) {
  ...
  const insights = useMemo(() => {
    ...
    const maxFeeds = Math.max(1, ...rows.map(r => r.feeds))

    const previousStart = new Date(days[0])
    previousStart.setDate(previousStart.getDate() - 7)
    const currentStart = days[0]
    const previousRated = sessions.filter(s => {
      const started = new Date(s.startedAt)
      return s.mood && started >= previousStart && started < currentStart
    })
    const previousAvgMood = previousRated.length
      ? previousRated.reduce((a, s) => a + s.mood, 0) / previousRated.length
      : null

    return { rows, totalFeeds, totalMeds, ratedFeeds, avgMood, previousAvgMood, avgFeedMins, maxFeeds }
  }, [sessions, nappies, medicines])

  const weeklyMood = moodSummary(insights.avgMood)
  const weeklyMoodTrend = moodTrend(insights.avgMood, insights.previousAvgMood)
    ...
    <div style={{ flex: 1, overflowY: 'auto', background: p.bg }}>
      ...
      <div style={{ padding: '0 14px 10px' }}>
        <button
          ...
          aria-label={showInsights ? 'Return to logbook entries' : 'View weekly logbook insights'}
          style={{ width: '100%', border: `1px solid ${p.border}`, borderRadius: 12, background: p.card, color: p.text, padding: '10px 12px', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}
        >
      ...
      {showInsights && (
        <div style={{ margin: '0 14px 12px', background: p.card, borderRadius: 14, border: `1px solid ${p.border}`, padding: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 120, marginBottom: 8 }}>
            {insights.rows.map(r => {
              const h = r.feeds ? Math.max(8, Math.round((r.feeds / insights.maxFeeds) * 96)) : 8
          ...
          <div style={{ background: p.bg, border: `1px solid ${p.border}`, borderRadius: 12, padding: '10px 12px', marginBottom: 10 }}>
            ...
            {weeklyMoodTrend && (
              <span style={{ display: 'block', fontSize: 11, color: p.sub, marginTop: 4 }}>{weeklyMoodTrend}</span>
            )}
          </div>
