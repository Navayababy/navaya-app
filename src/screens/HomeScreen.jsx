...
export default function HomeScreen({ night, onNightToggle, timer }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: p.bg }}>
      ...
      {/* ── Quick start help ── */}
      <div style={{ margin: '12px 14px 0', background: p.card, borderRadius: 14, border: `1px solid ${p.border}`, padding: '12px 14px' }}>
        ...
        <span style={{ display: 'block', fontSize: 13, color: p.text, lineHeight: 1.45 }}>
          Start a feed with Left or Right, then finish to save it in History. Need a full walkthrough?
        </span>
        <a
          href={appGuideUrl}
          target="_blank"
